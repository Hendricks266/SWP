...
//-------------------------------------------------------------------------
//
// Removed several initprintf's
// Added escape in waiting
// modified waitforeverybody(int x)
// Added usermap to packet
// Added useraddon to packet
//
...
void waitforeverybody(int x)
{
    long i, size = 1;
    short other;
    long stopclock = totalclock + 360;

    if (!CommEnabled)
        return;

    initprintf("waitforeverybody(%d) #%d\n", x, Player[myconnectindex].playerreadyflag + 1);
    ...
    while (TRUE)
    {
        ...
        if (KEY_PRESSED(KEYSC_ESC) || quitevent || (wfe_ExitCallback && wfe_ExitCallback()))
        {
            // allow exit
        ...
        if (gNet.MultiGameType == MULTI_GAME_COMMBAT && x == 1 && gNet.TimeLimitClock == 0 && totalclock > stopclock)
            return;
...
void AddSyncInfoToPacket(long *j)
    {
    long sb;
	int count = 0;
...
VOID getpackets(VOID)
    ...
    while ((packbufleng = netgetpacket(&otherconnectindex, packbuf)) > 0)
        ...
        case PACKET_TYPE_NEW_GAME:
            ...
            gNet.Skill        = p->Skill;
            gNet.MaxTime      = p->TimeLimit;

            gs.NetGameType    = p->GameType;
            gs.NetLevel       = p->Level;
            ...
            if (!bDone && strlen(p->useraddon) > 4)
            {
                strcpy(UserAddon, p->useraddon);
                initgroupfile(UserAddon);
                initprintf("Adding Packet Group: %s\n", UserAddon);
                 checksearchpath(UserAddon);
                strcpy(UserAddon, fname);
                checkforhrp(UserAddon, 0);
                LoadDefFiles(1);                       // load custom only
                bDone = TRUE;
            }

            if (strlen(p->usermap) > 4)
            {
                strcpy(UserMapName, p->usermap);
                if (checksearchpath(UserMapName) == -1)
                    addsearchpath("Maps");
                strcpy(UserMapName, fname);
                initprintf("Adding Packet Map: %s\n", UserMapName);
                checkforhrp(UserMapName, 1);
                LoadDefFiles(1);                       // load custom ony
            }

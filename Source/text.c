...
// Added PutStringInfo2
// Added Autosave key A
...
PANEL_SPRITEp pClearTextLineID(PLAYERp pp, short id, long y, short pri)
    {
...
PANEL_SPRITEp pMenuClearTextLineID(PLAYERp pp, short id, long y, short pri)
    {
    PANEL_SPRITEp nsp=NULL;
    PANEL_SPRITEp psp=NULL, next;
...
void PutStringTimer(PLAYERp pp, short x, short y, char *string, short seconds)
{
    ...
    if (Autotick > 0)
       {
       if (totalclock > Autotick - 30)  // screen text causes save errors
           return;
       }
    ...
    for (ndx = 0; (c = string[ndx]) != 0; ndx++)
        {
        ac = c - '!' + STARTALPHANUM;
        if( (ac < STARTALPHANUM || ac > ENDALPHANUM)  && c != asc_Space  && c != KEYSC_A)
            break;

        if (c > asc_Space && c < 127 && c != KEYSC_A)
            {
...
VOID DisplayPanelNumber(PLAYERp pp, short xs, short ys, int number)
    ...
    for (ptr = buffer, x = xs; *ptr; ptr++, x += size)
    {
        ...
        else
        {
            ...
            rotatesprite((long)x << 16, (long)ys << 16, (1 << 16), 0, pic, 0, 0,
...
VOID DisplayMiniBarNumber(PLAYERp pp, short xs, short ys, int number)
{
    ...
    {
         p = PANEL_FONT_G+*(b+k)-'0';
         rotatesprite((long)(c+j) << 16, (long)ys << 16, 52000, 0, p, 0, 0,
                      ROTATE_SPRITE_SCREEN_CLIP | ROTATE_SPRITE_CORNER, 0, 0, xdim - 1, ydim - 1);
         j += tilesizx[p];
    }
}
...
VOID DisplayMiniBarSmString(PLAYERp pp, short xs, short ys, short pal, CHARp buffer)
    ...
    #define FRAG_FIRST_TILE 2930   //exclamation point  4608
    ...
    for (ptr = buffer, x = xs; *ptr; ptr++, x += size)
        ...
        rotatesprite((long)x << 16, (long)ys << 16, (1 << 16), 0, pic, 0, pal,
            ROTATE_SPRITE_SCREEN_CLIP | ROTATE_SPRITE_CORNER, 0, 0, xdim - 1, ydim - 1);
        }
    }
...
VOID DisplayFragString(PLAYERp pp, short xs, short ys, CHARp buffer)
    {
    short size=4,x;
    CHARp ptr;
    PANEL_SPRITEp nsp;

...
PANEL_SPRITEp PanelNdxToSprite(PLAYERp pp, long ndx)
    {
    short count = 0;
...
int SaveSymDataInfo(MFILE fil, void *ptr)
{
	...
	if (Saveable_FindDataSym(ptr, &sym))
	{
		...
		fp = fopen("Savegame symbols.txt", "a");
		if (fp)
		{
			fprintf(fp,"data %p\n",ptr);
			fclose(fp);
...
int SaveGame(short save_num)
{
    ...
    if (save_num == 10)
        sprintf(game_name,"%sgamex.asv", svgame);
    else
    ...
    if (save_num == 10)
        strcpy(SaveGameDescr[10],"Auto Saved Game");
    ...
    MWRITE(&cdatrack,sizeof(cdatrack),1,fil);
    ...
    if (saveisshot)
        initprintf("Problem saving to: %s\n", game_name);

    return(saveisshot?-1:0);
}

int LoadGameFullHeader(short save_num, char *descr, short *level, short *skill, char *Mapname)
{
    MFILE fil;
    char game_name[80];
    short tile;
...
int LoadGame(short save_num)
{
    ...
    MREAD(&cdatrack,sizeof(cdatrack),1,fil);

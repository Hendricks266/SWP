...
// Added Autosave
// Added Stopsound to underwater dive
// Added timeclock to zero
...
VOID DoPlayerDiveMeter(PLAYERp pp)    // StopSound();
{
...
DoPlayerBeginDie(PLAYERp pp)
    ...
    if (!CommEnabled && numplayers <= 1 && QuickLoadNum >= 0)
        ...
        if (Autodone == 1 && numplayers <= 1)
            PutStringInfoLine2(pp, "Press 'A' to load Autosaved Game");
...
VOID DoPlayerDeathCheckKeys(PLAYERp pp)
{
    ...
    if (Autodone == 1 && (KB_KeyPressed(sc_A) || BUTTON(gamefunc_Open)))
    {
       KB_ClearKeysDown();
       if (LoadGame(10) == -1)
           ResumeAction();
...
VOID MultiPlayLimits(VOID)
{
    ...
    if (Done)
    {
        gNet.TimeLimitClock = 0; //gNet.TimeLimit;

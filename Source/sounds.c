...
// Added gs.Talking
// Added force playsound if statscreen
// Added play WAV files
// Fixed sound crashes
//
...
int PlaySong(char *song_file_name, int cdaudio_track, BOOL loop, BOOL restart)
{
    ...
    if (strstr(song_file_name, ".wav"))
    {
        StopSound();

        if (!LoadSong(song_file_name))
            return(MUSIC_Error);

        return(MV_PlayLoopedWAV(SongPtr, 0, SongLength, 0,   255, 255, 255, FX_MUSIC_PRIORITY, MUSIC_ID));
    }
    else
    {
...
//  MONO_PRINT(ds);
    ...
    {
        extern BOOL bStatScreen;
        extern BOOL bHosting;

        if ((CommEnabled && bHosting && bStatScreen) || (tx==0 && ty==0 && tz==0)) // It's a non-inlevel sound wxwb
        {
...
void TestMusic( void )
   ...
   if ( status == MUSIC_Ok )
      ...
      status = PlaySong("theme.mid", -1, TRUE, TRUE);

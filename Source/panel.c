...
PANEL_SPRITEp pFindMatchingSprite(PLAYERp pp, long x, long y, short pri)
    {
    PANEL_SPRITEp nsp;
...
PANEL_SPRITEp pFindMatchingSpriteID(PLAYERp pp, short id, long x, long y, short pri)
    {
    PANEL_SPRITEp nsp;
    PANEL_SPRITEp psp=NULL, next;
...
VOID PlayerUpdateAmmo(PLAYERp pp, short UpdateWeaponNum, short value)
{
    ...
    {
        if (gs.BorderNum < BORDER_MINI_BAR || pp - Player != screenpeek)
            return;
...
VOID PlayerUpdateWeaponSummary(PLAYERp pp, short UpdateWeaponNum)
{
    ...
    if (TEST(pp->WpnFlags, BIT(WeaponNum)))               // display 2: 3: 4: etc
...
VOID PlayerUpdateArmor(PLAYERp pp, short value)
{
    ...
    if (gs.BorderNum == BORDER_MINI_BAR && gs.MiniHudTyp == 1)
        DisplayMiniBarNumber(pp, 38, 186, pp->Armor); // amount
    else
    if (gs.BorderNum == BORDER_MINI_BAR && gs.MiniHudTyp == 0)
        return;
    else
        DisplayPanelNumber(pp, x, y, pp->Armor); // amount
}

BOOL PlayerUpdateKeys(PLAYERp pp)
{
    ...
    short x,y;
    short row,col;
    short i, j, xsize, ysize;                                        // NB: no keys in wb
    BOOL bFound;
    extern long ScreenBPP;
    ...
    xsize = tilesizx[PANEL_KEY_RED]+1;                               // main screen
    ysize = tilesizy[PANEL_KEY_RED]+2;

    tilesizy[2402] = 17;    // 22 x 19                               // blank if (!CommEnabled) ?
    ...
    if (gs.BorderNum == BORDER_MINI_BAR)
    {
        ...
        for (i=0; i<4; i++)
        {
             if (pp->HasKey[i])    // keycards
             {
                ...
                rotatesprite(x << 16, y << 16, 52000L, 0, StatusKeyPics[i], 0, 21, 10+16, 0, 0, xdim - 1, ydim - 1);
                bFound = TRUE;
             }
        }
        for (i=0; i<4; i++)
        {
             if (pp->HasKey[i+4])  // skeleton keys
             {
                x = 305;
                y = 192 - (i * 4);
                ...
                rotatesprite(x << 16, y << 16, 52000L, 0, StatusKeyPics[i+4], 0, 21, 10+16, 0, 0, xdim - 1, ydim - 1);
                bFound = TRUE;
             }
        }
    }
    ...
    {
        pSpawnFullScreenSprite(pp, KEYS_ERASE, PRI_MID, PANEL_KEYS_BOX_X, PANEL_BOX_Y+1);
        i = 0;
        for (row = 0; row < 2; row++)
        {
            for (col = 0; col < 2; col++)
...
VOID SpecialUziRetractFunc(PANEL_SPRITEp psp)
    ...
    if (psp->y >= 200 + tilesizy[psp->picnum])
        {
        pKillSprite(psp);
        }
    }
...
pSwordHide(PANEL_SPRITEp psp)
    ...
    if (psp->y >= SWORD_YOFF + tilesizy[picnum])
        {
        psp->y = SWORD_YOFF + tilesizy[picnum];
        psp->x = SWORD_XOFF;
...
pStarHide(PANEL_SPRITEp psp)
    ...
    if (psp->y >= STAR_YOFF + tilesizy[picnum])
        {
        psp->y = STAR_YOFF + tilesizy[picnum];
...
void pUziOverlays(PANEL_SPRITEp psp, short mode)
    {
...
pUziReloadRetract(PANEL_SPRITEp nclip)
    ...
    if (gun->y > UZI_RELOAD_YOFF + tilesizy[gun->picndx])
        {
...
pUziHide(PANEL_SPRITEp psp)
    ...
    if (psp->y >= 200 + tilesizy[picnum])
        {
        psp->y = 200 + tilesizy[picnum];
...
void pUziFire(PANEL_SPRITEp psp)
    {
    PLAYERp pp = psp->PlayerP;
...
SpawnUziShell(PANEL_SPRITEp psp)
    ...
    shell->y = shell->yorig = psp->y - tilesizy[psp->picndx] + 20;
...
BOOL pShotgunOverlays(PANEL_SPRITEp psp)
    {
...
pShotgunHide(PANEL_SPRITEp psp)
    ...
    if (psp->y >= SHOTGUN_YOFF + tilesizy[picnum])
        {
        psp->y = SHOTGUN_YOFF + tilesizy[picnum];
...
pShotgunRetract(PANEL_SPRITEp psp)
    ...
    if (psp->y >= SHOTGUN_YOFF + tilesizy[picnum] + 50)
        {
        RESET(psp->PlayerP->Flags, PF_WEAPON_RETRACT);
...
pRailHide(PANEL_SPRITEp psp)
    ...
    if (psp->y >= RAIL_YOFF + tilesizy[picnum])
        {
        psp->y = RAIL_YOFF + tilesizy[picnum];
...
pRailRetract(PANEL_SPRITEp psp)
    ...
    if (psp->y >= RAIL_YOFF + tilesizy[picnum] + 50)
        {
        RESET(psp->PlayerP->Flags, PF_WEAPON_RETRACT);
        psp->PlayerP->Wpn[psp->WeaponType] = NULL;
...
pHotheadHide(PANEL_SPRITEp psp)
    ...
    if (psp->x >= HOTHEAD_XOFF + tilesizx[picnum] || psp->y >= HOTHEAD_YOFF + tilesizy[picnum])
        {
        psp->x = HOTHEAD_XOFF;
        psp->y = HOTHEAD_YOFF + tilesizy[picnum];
...
pHotheadRetract(PANEL_SPRITEp psp)
    ...
    if (psp->y >= HOTHEAD_YOFF + tilesizy[picnum])
        {
        RESET(psp->PlayerP->Flags, PF_WEAPON_RETRACT);
...
pMicroHide(PANEL_SPRITEp psp)
    ...
    if (psp->y >= MICRO_YOFF + tilesizy[picnum] + 20)
        {
        psp->y = MICRO_YOFF + tilesizy[picnum] + 20;
        psp->x = MICRO_XOFF;
...
BOOL pMicroOverlays(PANEL_SPRITEp psp)
{
...
pMicroRetract(PANEL_SPRITEp psp)
    ...
    if (psp->y >= MICRO_YOFF + tilesizy[picnum])
        {
        RESET(psp->PlayerP->Flags, PF_WEAPON_RETRACT);
...
pHeartHide(PANEL_SPRITEp psp)
    ...
    if (psp->y >= HEART_YOFF + tilesizy[picnum])
        {
        psp->y = HEART_YOFF + tilesizy[picnum];
...
pMineLower(PANEL_SPRITEp psp)
    ...
    if (psp->y > MINE_YOFF + tilesizy[psp->picndx])
        {
        if (!WeaponOK(psp->PlayerP))
            return;
        psp->y = MINE_YOFF + tilesizy[psp->picndx];
        pStatePlusOne(psp);
        }
    }
...
pMineHide(PANEL_SPRITEp psp)
    ...
    if (psp->y >= MINE_YOFF + tilesizy[picnum])
        {
        psp->y = MINE_YOFF + tilesizy[picnum];
        psp->x = MINE_XOFF;
...
pChopsRetract(PANEL_SPRITEp psp)
    ...
    if (psp->y >= CHOPS_YOFF + tilesizy[picnum])
        {
        if(RANDOM_RANGE(1000) > 800)
...
void SpawnFistBlur(PANEL_SPRITEp psp)
    {
    PANEL_SPRITEp nsp;
    //PICITEMp pip;
...
void pFistHide(PANEL_SPRITEp psp)
    ...
    if (psp->y >= FIST_YOFF + tilesizy[picnum])
        {
        psp->y = FIST_YOFF + tilesizy[picnum];
...
VOID pDisplaySprites(PLAYERp pp)
{
    USERp u = User[pp->PlayerSprite];
    ...
    TRAVERSE(&pp->PanelSpriteList, psp, next)
    {
        ...
        // 2080 = sword
        // 4090 = sword with blood
        // 4050 = fists
        // 4051 = fists with blood
        // 4070 = fists
        // 4077 = fists with blood
        // 2130 - 2138 = shuriken
        // 2510 - 2518 = darts
        // 2211 = 2216 = riotgun
        // 2225 - 2227 = riotgun alt
        // 2004 - 2008 = uzi
        // 2070 - 2078 = rpg
        // 2121 - 2126 = grenade launcher
        // 2220 = sticky closed
        // 2221 = sticky ready
        // 2010 - 2020 = railgun
        // 2048 - 2949 = guru
        ...
        if (gs.WeaponHide)
        {
            short hides[72] = {2080,4090,4050,4051,4070,4077,2130,2131,2132,2134,2135,2136,2137,2138,
                               2510,2511,2512,2514,2515,2516,2517,2518,2211,2212,2213,2214,2215,2216,
                               2225,2226,2227,2004,2006,2008,2070,2071,2072,2073,2074,2075,2076,2077,
                               2078,2121,2122,2123,2124,2125,2126,2220,2221,2010,2011,2012,2013,2014,
                               2015,2016,2017,2018,2019,2020,2048,2049,2050,2051,2052,2053,2054,2055,2056,2057};
            short i;
        ...
        rotatesprite(x << 16, y << 16, psp->scale, ang, picnum, shade, pal, flags, x1, y1, x2, y2);
...
hideme:
        ...
        for (i = 0; i < SIZ(psp->over); i++)
        {
            ...
            if (picnum)
            {
                if (gs.WeaponHide)
                {
                    if (picnum >= UZI_COPEN && picnum <= UZI_CRELOAD)
                        break;
                    if (picnum == SHOTGUN_AUTO || picnum == MICRO_HEAT)
                        break;
                    if (picnum == MICRO_SIGHT || picnum == MICRO_SHOT_20 || picnum == MICRO_SHOT_1)
                        break;
                    if (picnum >= HEAD_MODE1 && picnum <= HEAD_MODE3)
                        break;
                }
                rotatesprite((x + psp->over[i].xoff) << 16, (y + psp->over[i].yoff) << 16, psp->scale, ang,

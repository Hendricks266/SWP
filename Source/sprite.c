...
// Added WeaponSwitch
// Added Racecar hack in $bullet
// Fixed Opposite WeaponSwitch
// Added car hack option
// Added "Dart" name pickup
// Added Autosave
// Fixed (c) in fortune cookies
// Fixed sprite crash if secnum < 0
// Fixed sprite crash in _ST1.map
...
SHORT SpawnSprite(short stat, short id, STATEp state, short sectnum, int x, int y, int z, int init_ang, int vel)
{
    SPRITEp sp;
    SHORT SpriteNum;
    USERp u;
    ...
    case ST1:
        ...
        if (tag < 16)
            ...
            case TRIGGER_SECTOR:
                ...
                if (gs.UseCarHack)
                   {
                   if (Bstrncasecmp(LevelName, "$bullet", 7) == 0)
                      {
                      if (SP_TAG7(sp) == 11)
                          SP_TAG7(sp) = 13;
                      }
...
DoGet(short SpriteNum)
    ...
    TRAVERSE_CONNECT(pnum)
        ...
        case ICON_STAR:
            ...
            sprintf(ds, DamageData[WPN_STAR].weapon_name);
            if (useDarts)
                sprintf(ds,"Darts");

...
PlayerOperateEnv(PLAYERp pp)
    ...
    if (sectu && sectu->damage)
        ...
        if ((SPRITEp_BOS(pp->SpriteP) >= sectp->floorz) && !TEST(pp->Flags, PF_DIVING))
            {
            PlayerTakeSectorDamage(pp);
            }
        }

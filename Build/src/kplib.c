...
static long bytesperline, xres, yres, globxoffs, globyoffs;
...
static inline void rgbhlineasm(long x, long xr1, intptr_t p, long ixstp)
{
    long i;
    if (!trnsrgb)
    {
...
static inline void pal8hlineasm(long x, long xr1, intptr_t p, long ixstp)
{
...
kpngrend_goodret:;
    if (!(filterest&~(1<<filter1st))) filtype = (signed char)filter1st;
...
static void yrbrend(long x, long y, long *ldct)
{
    long i, j, ox, oy, xx, yy, xxx, yyy, xxxend, yyyend, yv, cr = 0, cb = 0, *odc, *dc, *dc2;
    intptr_t p, pp;
...
void (*kplib_yrbrend_func)(long,long,long *) = yrbrend;

static long kpegrend(const char *kfilebuf, long kfilength,
                        intptr_t daframeplace, long dabytesperline, long daxres, long dayres,
                        long daglobxoffs, long daglobyoffs)
{
    long i, j, v, leng, xdim = 0, ydim = 0, index, prec, restartcnt, restartinterval;
    long x, y, z, xx, yy, zz, *dc = NULL, num, curbits, c, daval, dabits, *hqval, *hqbits, hqcnt, *quanptr = NULL;
        ...
        case 0xc4:  //Huffman table
            do
            {
                ...
                for (i=1; i<=16; i++)
                {
                    ...
                    Bmemcpy((void *)&huftable[index][hufcnt[index]],(void *)kfileptr,(long)hufnumatbit[index][i]);
                    if (i <= 10)
                        ...
                        long daglobxoffs, long daglobyoffs)
{
    ...
    if ((xoff != 0) || (yoff != 0) || (xsiz != xspan) || (ysiz != yspan))
    {
        ...
        for (y=yy[0]; y<yy[1]; y++,lptr=(long *)(((intptr_t)lptr)+dabytesperline))
            for (x=xx[0]; x<xx[3]; x++) lptr[x] = backcol;
        for (; y<yy[2]; y++,lptr=(long *)(((intptr_t)lptr)+dabytesperline))
        {
            for (x=xx[0]; x<xx[1]; x++) lptr[x] = backcol;
            for (x=xx[2]; x<xx[3]; x++) lptr[x] = backcol;
        }
        for (; y<yy[3]; y++,lptr=(long *)(((intptr_t)lptr)+dabytesperline))
            for (x=xx[0]; x<xx[3]; x++) lptr[x] = backcol;
...
//                        旼컴컴컴컴컴컴컴?
//                        ? 0(2): "BM"   ?
// 旼컴컴컴컴컴컴컴컴컴컴엿 10(4): rastoff?旼컴컴컴컴컴컴컴컴커
// ?eadsiz=12 (OS/2 1.x)납 14(4): headsiz??All new formats: ?
//憫컴컴컴컴컴컴컴컴컴컴컨좔컴컴컴컴컴컴쩡좔좔컴컴컴컴컴컴컴컴컨컴컴컴컴컴컴컴컴컴컴컴커
//?18(2): xsiz                         ?18(4): xsiz                                  ?
//?20(2): ysiz                         ?22(4): ysiz                                  ?
...
//읕컴컴컴컴컴컴컴컴컴컴쩡컴컴컴컴컴컴컴좔컴컴컴컴쩡컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴컴켸
//                      ?rastoff(?): bitmap data ?
//                      읕컴컴컴컴컴컴컴컴컴컴컴컴?
                        ...
                        long daglobxoffs, long daglobyoffs)
{
    ...
    if (cdim < 16)
    {
        ...
        coltype = 3; bitdepth = (signed char)cdim; paleng = i; //For PNGOUT
    }
    else if (!(cdim&15))
    {
        coltype = 2;
...
static long kpcxrend(const char *buf, long fleng,
                        ...
                        long daglobxoffs, long daglobyoffs)
{
    long i, j, x, y, nplanes, x0, x1, y0, y1, bpl, xsiz, ysiz;
    intptr_t p;
...
static long kddsrend(const char *buf, long leng,
                        intptr_t frameptr, long bpl, long xdim, long ydim, long xoff, long yoff)
{
    long x, y, z, xx, yy, xsiz, ysiz, dxt, al[2], ai, k, v, c0, c1, stride;
    intptr_t j;
    ...
    if ((*(long *)&buf[80])&LSWAPIB(64)) //Uncompressed supports only A8R8G8B8 for now
    {
        ...
        if ((*(unsigned long *)&buf[104]) != LSWAPIB(0xff000000)) return(-1);
        buf += 128;
...
long kprender(const char *buf, long leng, intptr_t frameptr, long bpl,
...
static long kzhashead[256], kzhashpos, kzlastfnam, kzhashsiz, kzdirnamhead = -1;
...
static long kzcheckhash(const char *filnam, char **zipnam, long *fileoffs, long *fileleng, char *iscomp)
{
    long i;
    ...
    for (i=kzhashead[kzcalchash(filnam)]; i>=0; i=(*(long *)&kzhashbuf[i]))
        if (!filnamcmp(filnam,&kzhashbuf[i+21]))
        {
            ...
            (*fileoffs) = *(long *)&kzhashbuf[i+12];
            (*fileleng) = *(long *)&kzhashbuf[i+16];
...
long kzaddstack(const char *filnam)
{
    FILE *fil;
    ...
    if (!fil) //if file not found, assume it's a directory
    {
        ...
        *(long *)&kzhashbuf[kzhashpos] = kzdirnamhead; kzdirnamhead = kzhashpos;
    ...
    if (i == LSWAPIB(0x04034b50)) //'PK\3\4' is ZIP file id
    {
        ...
        for (i=0; i<numfiles; i++)
        {
            ...
            *(long *)&kzhashbuf[kzhashpos+12] = LSWAPIB(*(long *)&tempbuf[42]); //fileoffs
            *(long *)&kzhashbuf[kzhashpos+16] = 0; //fileleng not used for ZIPs (reserve space for simplicity)
            *(long *)&kzhashbuf[kzhashpos+20] = 1; //iscomp
    ...
    else if (i == LSWAPIB(0x536e654b)) //'KenS' is GRP file id
    {
        ...
        if ((*(long *)&tempbuf[0] != LSWAPIB(0x65766c69)) || //'ilve'
                (*(long *)&tempbuf[4] != LSWAPIB(0x6e616d72)))   //'rman'
            { fclose(fil); return(0); }
        numfiles = LSWAPIB(*(long *)&tempbuf[8]); k = ((numfiles+1)<<4);
        for (i=0; i<numfiles; i++,k+=leng)
        {
            fread(tempbuf,16,1,fil);
            leng = LSWAPIB(*(long *)&tempbuf[12]); //File length
            tempbuf[12] = 0;
            ...
            *(long *)&kzhashbuf[kzhashpos+4] = kzlastfnam;
            *(long *)&kzhashbuf[kzhashpos+8] = zipnamoffs;
            *(long *)&kzhashbuf[kzhashpos+12] = k; //fileoffs
            *(long *)&kzhashbuf[kzhashpos+16] = leng; //fileleng
            *(long *)&kzhashbuf[kzhashpos+20] = 0; //iscomp
...
intptr_t kzopen(const char *filnam)
{
    ...
    for (i=kzdirnamhead; i>=0; i=*(long *)&kzhashbuf[i])
    {
...
kzfindfile_beg:;
    ...
    while (srchstat == 2)
    {
        ...
        if (wildmatch(&kzhashbuf[srchzoff+21],newildst))
        {
            ...
            srchzoff = *(long *)&kzhashbuf[srchzoff+4];
            return(1);
        }
        srchzoff = *(long *)&kzhashbuf[srchzoff+4];
    }
    while (srchstat == 3)
    {
        ...
        srchdoff = *(long *)&kzhashbuf[srchdoff];
...
kzreadplc1:;         i = kzfs.i; bfinal = kzfs.bfinal;
                    ...
                    slidebuf[(gslidew++)&32767] = (unsigned char)getbits(8);
                }
                continue;
            }
            if (btype == 3) continue;
...
kzreadplc2:;      bfinal = kzfs.bfinal;
                ...
                if (i < 256) { slidebuf[(gslidew++)&32767] = (unsigned char)i; continue; }
                if (i == 256) break;
                i = getbits(hxbit[i+30-257][0]) + hxbit[i+30-257][1];
...
void kpzload(const char *filnam, intptr_t *pic, long *bpl, long *xsiz, long *ysiz)
{
    char *buf;
    long leng;
    long handle = kopen4load((char *)filnam, 0);

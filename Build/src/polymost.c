...
// Added Refix voxels
// Added glfovscreen,glwidescreen
// Added alphahackarray
// Added vsync
// Added aspect ration
//
...
void resizeglcheck()
{
    ...
    if ((glox1 != windowx1) || (gloy1 != windowy1) || (glox2 != windowx2) || (gloy2 != windowy2))
    {
        ...
        fovtemp = (int)((windowx2-windowx1+1) * 1.05) - (windowx2-windowx1+1);
        ratio = ratio + ((float)glfovscreen / 20);

        fovcorrect = (int)(ph?(glwidescreen?0:(((windowx2-windowx1+1) * ratio) - (windowx2-windowx1+1))):0);
        if (glwidescreen)
            fovcorrect = fovcorrect + fovtemp * glfovscreen;
        bglViewport(windowx1 - (fovcorrect / 2), yres-(windowy2+1),windowx2-windowx1+1 + fovcorrect, windowy2-windowy1+1);

        bglMatrixMode(GL_PROJECTION);
        memset(m,0,sizeof(m));
        m[0][0] = (float)ydimen / (ph?ratio:1.f); m[0][2] = 1.0;
  		m[1][1] = (float)xdimen; m[1][2] = 1.0;
        m[2][2] = 1.0; m[2][3] = (float)ydimen / (ph?ratio:1.f);
        m[3][2] =-1.0;

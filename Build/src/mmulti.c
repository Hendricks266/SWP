// Added multiplayer stuff
// Added force player join to exit and not hang
//
...
long initmultiplayersparms(long argc, const char **argv)
{
	...
	for(i=0;i<argc;i++)
	{
		...
		if (isvalidipaddress(st))
		{
			...
			for(j=0;st[j];j++)
			{
				if (st[j] == ':')
					{
					    otherport[daindex] = htons((unsigned short)atol(&st[j+1]));
					    st[j] = 0;
					    break;
					}
			}
		...
		else
		{
			...
			for (j=0;st[j];j++)
				if (st[j] == ':')
				{
				    pt = htons((unsigned short)atol(&st[j+1]));
				    st[j] = 0;
				    break;
				}
			if ((lph = gethostbyname(st)))
			{
			    ...
			    printf("mmulti: Failed resolving %s\n",argv[i]);
		}
		free(st);
	}
	if ((danetmode == 255) && (daindex))
	   {
	    numplayers = 2;
	    danetmode = 0;
	   } //an IP w/o /n# defaults to /n0
	if ((numplayers >= 2) && (daindex) && (!danetmode))
	    myconnectindex = 1;
...
long initmultiplayerscycle(void)
{
	...
	else
	{
		...
		if (tims >= lastsendtims[connecthead]+200) //1000/PAKRATE)
		{
			lastsendtims[connecthead] = tims;
			k = 2;
			*(long *)&pakbuf[k] = -1; k += 4;
			...
			*(unsigned short *)&pakbuf[k] = getcrc16(pakbuf,k); k += 2;
			...
			netready = 1;         // force a return else sw hangs here wxnh
			return 0;
		}
	}
...
void initmultiplayers (long argc, const char **argv, char damultioption, char dacomrateoption, char dapriority)
{
	long i, j, k, otims;

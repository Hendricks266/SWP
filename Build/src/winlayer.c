...
//
// Written for DirectX 7.0
//-------------------------------------------------------------------

// Added is_vista
// Added increase to wm_msgbox buffer
// Removed Detecting video modes:
// Added ShellExecute
// Added DeleteAutosave
// Added startup menu SetCommandline
// Added darker gamma - LcdMon
// Improved horiz mouse jitter
// Added check for windows 7 is_vista = 2
// Added refreshrate to log on video setting
// Added fix for 8 bit mode in win7
// Added extra buffersize to initprintf
// Added vsync
// 4.2.9.5.7
// Added string.h
// Added isync
// Added new DeleteAutosave
//
...
int wm_msgbox(char *name, char *fmt, ...)
{
    ...
    if (is_vista > 0)
    	MessageBox(hWindow,buf,name,MB_OK|MB_TASKMODAL|MB_SETFOREGROUND|MB_TOPMOST);
    	//MessageBox(NULL, "Messagebox test", "MESSAGEBOX TEST", MB_SETFOREGROUND | MB_OK);
    else
        MessageBox(hWindow,buf,name,MB_OK|MB_TASKMODAL); //|MB_ICONINFORMATION
	    //MessageBoxEx(hWindow,buf,name,MB_OK|MB_TASKMODAL,LANG_ENGLISH);
	return 0;
}
...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE hPrevInst, LPSTR lpCmdLine, int nCmdShow)
{
	...
	if (instanceflag)
	    CloseHandle(instanceflag);

	if (argvbuf)
	    free(argvbuf);

    if (r == 1)
        ShellExecute(NULL, "Open", "SWP.exe", "-setup", NULL, SW_SHOWNORMAL);
...
int SetCommandLine(char *argvbufx)
{
	...
	if (argvbufx)        // carve up the commandline into more recognizable pieces
    {
		for (p=wp=argvbufx; *p; p++)
	    {
			if (*p == ' ')
  		    {
				if (instring && !quoted)
...
{
	va_list va;
	char buf[2000]; //,*p=NULL,*q=NULL,workbuf[1024];

	va_start(va, f);
	Bvsnprintf(buf, 2000, f, va);
	va_end(va);
	OSD_Printf(buf);
...
static void ProcessInputDevices(void)
{
					...
					for (i=0; i<dwElements; i++)
					{
						switch (didod[i].dwOfs)
						{
							...
							case DIMOFS_Z:
								...
								if ((int)didod[i].dwData < 0)  // wheel down
								{
									if (mousewheel[1] > 0 && mousepresscallback) mousepresscallback(6,0);
									mousewheel[0] = t;
	...
	if (lastKeyDown > 0)
	{
		...
		if ((u >= 30) && (lastKeyDown&0x80000000l))
		{
...
int setvideomode(int x, int y, int c, int fs)
{
	...
	initprintf("Setting video mode %dx%d (%d-bpp %s) at %d hz refreshrate.\n",
			x,y,c, ((fs&1) ? "fullscreen" : "windowed"), maxrefreshfreq);
...
void InitVSync()
{
    char* extensions = (char*)bglGetString(GL_EXTENSIONS);
...
void getvalidmodes(void)
{
	...
	{
		{1920,1440},{1920,1200},{1600,1200},{1280,1024},{1280,960},{1152,864},{1024,768},
		{800,600},{640,480},{640,400},{512,384},{480,360},{400,300},{320,240},{320,200},{0,0}
	};
	int cdepths[2] = { 8, 0 };
...
//		result = IDirectDrawPalette_SetEntries(lpDDPalette, 0, start, 256, (LPPALETTEENTRY)&lpal.palPalEntry[start]);
		if (result != DD_OK)
		{
...
int setgamma(float ro, float go, float bo)
{
	...
	ro = gr / ro; go = gr / go; bo = gr / bo;                        //  (was all 1.0)
...
static void UninitDirectDraw(void)
{
    ...
    bDDrawInited = FALSE;     // move here to stop crash at exit
...
static int RestoreDirectDrawMode(void)
{
	...
	if (modesetusing == 0)
	{
		...
		if (result != DD_OK)
		{
			ShowDDrawErrorBox("Error setting co-operative level", result);
			UninitDirectDraw();
			return TRUE;
		}
	}
...
static int SetupOpenGL(int width, int height, int bitspp)
{
...

	{
		...
		while ((p2 = Bstrtoken(p3==p?p:NULL, " ", (char**)&p3, 1)) != NULL)
		{
			...
			if (!Bstrcmp(p2, "GL_EXT_texture_edge_clamp") ||
			...
			if (!Bstrcmp(p2, "GL_EXT_bgra"))
			{
				// support bgra textures
				glinfo.bgra = 1;
			}
			else
			if (!Bstrcmp(p2, "GL_ARB_texture_compression"))
			{
				// support texture compression
				glinfo.texcompr = 1;
			}
			else
			if (!Bstrcmp(p2, "GL_ARB_texture_non_power_of_two"))
			{
				// support non-power-of-two texture sizes
				glinfo.texnpot = 1;
			}
			else
			if (!Bstrcmp(p2, "WGL_3DFX_gamma_control"))
			{
...
static LRESULT CALLBACK WndProcCallback(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	switch (uMsg)
	{
        ...
        case WM_PALETTECHANGED:
            ...
            if (is_vista > 0 && bDDrawInited && bpp == 8 && fullscreen)
            {
            ...
            if (appactive && (HWND)wParam != hWindow)
                setpalette(0,256,0);
            break;
...
{
    GetCurrentDirectory(128, folder);
}
